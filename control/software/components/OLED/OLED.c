...
void OLED_ShowNum(uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size2)
{
    ...
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))
        {
            if (temp == 0)
            {
                OLED_ShowChar(x + (size2 / 1.2) * t, y, ' ', size2);
                continue;
            }
            else
                enshow = 1;
        }
        OLED_ShowChar(x + (size2 / 1.2) * t, y, temp + '0', size2);
    }
}
...
{         	
	...
	//小数点
	OLED_ShowChar(x+(size2/1.2)*(z_len),y,'.',size2); 
  ...
  //小数部分
	for(t=0;t<f_len;t++)
	{
		...
		OLED_ShowChar(x+(size2/1.2)*(t+z_len)+5,y,temp+'0',size2); 
	}
}